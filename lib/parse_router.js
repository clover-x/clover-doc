...
const parseDS = require('./data_struct_generator.js');
...
function parseFile(file, parent = '', swaggerDoc) {
    ...
    while((definition = jsDocRegex.exec(content)) !== null) {
        ...
        let tags = definition['tags'] ? definition['tags'].split(',').map(item => _.trim(item)) : [];
        ...
        swaggerDoc.definitions[responseModel] = _.defaultsDeep(
            ...
            {
                properties: {
                    data: parseDS(definition.response)
                }
            }
        );
        ...
        // 绑定
        let finalPath = `${parent ? '/' + parent : ''}/${path.basename(file, '.js')}${definition.path}`;
        swaggerDoc.paths[finalPath] = Object.assign(swaggerDoc.paths[finalPath] || {}, apiDef);
    }
}
...
function parsePath(baseDir, swaggerDoc, parent = '') {
    ...
    routers.forEach(function (item) {
        ...
        if(itemStats.isDirectory()) {
            parsePath(baseDir, swaggerDoc, path.join(parent, item));
